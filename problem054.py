...
From http://projecteuler.net/index.php?section=problems&id=54
"""

def problem054(lines):
    player_1_win_count = 0
    for line in lines:
        player_1 = line.split()[:5]
        player_2 = line.split()[5:]
        player_1 = sort_hand(player_1)
        player_2 = sort_hand(player_2)
        # TODO Far from complete
    return player_1_win_count

def sort_hand(hand):
    """
    >>> sort_hand(['8D', '3S', '5D', '5C', 'AH'])
    ['3S', '5D', '5C', '8D', 'AH']
    >>> sort_hand(['8D', '3S', '5C', '5D', 'AH'])
    ['3S', '5C', '5D', '8D', 'AH']
    """

    return sorted(hand, cmp=card_value_cmp, key=lambda s: s[0])

VALUES = '23456789TJQKA'
def card_value_cmp(a, b):
    """
    >>> card_value_cmp('2', '8')
    -1
    >>> card_value_cmp('K', '8')
...
""".split('\n')[1:-1]
    assert len(five_hands) == 5
    assert problem054(five_hands) == 3
    thousand_hands = """
...
""".split('\n')[1:-1]
    print problem054(thousand_hands)
